...
export const works = [
  {
    ...
    description: (
      <>
        <div>
          <p>
            Разработал множество сервисов (микрофронтенд и полноценные
            приложения):{" "}
            <b>
              Сертификация данных, Ввод/корректировка данных,
              DataLineage(движение данных)(микрофронтенд), Сервис
              уведомлений(микрофронтенд), Панель
              администрирования(микрофронтенд)
            </b>
          </p>

          <p>
            На <b>Сертификация данных, Ввод/корректировка данных</b> ввел code
            review, единый стиль для работы с ветками. Занимался как написанием
            кода, так и помощью коллегам. Вместе с бизнес требованиями выполнена
            большая работа по <b>рефакторингу</b> и оптимизации
            производиьельности приложения (в некотрых местах прирост от 3-7
            раз). Помимо оптимизации кода сделано обновление сборки, ускорена
            подача статики, уменьшен размер бандла (размер ~ 55%, скороксть до
            2.5 раз, для отдельных страниц сделан SSG)
          </p>

          <p>
            Раработка графовой платформы для визуализации направленных данных с
            вложенными вершинами(<b>DataLineage</b>). Придумал архитектуру,
            структуру хранения проиндексированных данных, группировку даных по
            уровням, возможность создания сквозного графа. С нуля сделан
            функионал приложения визуализации с возможностью рассчета и
            отрисовки порядка 50.000 вершин и связей. Все рассчеты написаны мной
            и выполняются в два процесса внутри WebWorker, что позволило делать
            рассчеты на большом количестве данных.
          </p>
          <p>
          ...
          <p>
            Паралельно усилил смежную команду и помогал в разработке двух
            микрофронтенд приложений
            <b> Сервис уведомлений, Панель администрирования</b>
          </p>
        </div>
        {[
          "ts",
  ...
  },
  {
    ...
    description: (
      <>
        <p>
          Разработал <b>конструктора</b> (плюс админки) лендингов (аналог
          Tilda). Придумал модель риализации приложения.
        </p>
        <p>Разработал клиента для работы с аналитикой, порядка 80ти экранов</p>
        <p>
          Учавствовал в написании основного сайта и мобильной версии,
          поддерживал и создавал внутренние проекты (партнерские программы,
          сервисы для работы с различными данными, парсеры). Создал
          <b>
            {" "}
            парсера, который парсит различные бк и выбирает оттуда даные
            (puppeteer, effector)
          </b>
          , делает отчет и отправляет в kafka и в graylog. Создал мобильное
...
export const paralelWorks = [
  {
    title: "AI-помощник для 1С-аналитиков",
    subTitle: <p>март 2025</p>,
    status: "finish",
    description: (
      <>
        <p>
          Разработка стартапа для 1с аналитиков, полностью с нуля написал
          конструктор для 1с компонентов. В нем можно создавать полные аналоги
          настоящих компонентов, которые полностью интерактивные и настраиваемые
          под конкретную задачу, поддержка слоев и группировок как в фигме.
          Написал обертку вокруг React flow, которая позволила отрисовывать
          тысячи элементов
          <span>
            <a href="https://1yes.pro/" rel="noreferrer" target="_blank">
              1yes.pro
            </a>
          </span>
        </p>
        <p>
          Так же сделал страницу-опросник для интервьювера, с возможностью
